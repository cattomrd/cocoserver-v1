...
async function loadPlaylists(filter = 'all') {
    ...
    try {
        ...
        // Actualizar contador de playlists de manera segura
...
async function createPlaylist(playlistData) {
    try {
        ...
        // Cerrar el formulario de manera segura
        if (window.bootstrap) {
            try {
...
// Abrir detalles de una playlist
async function openPlaylistDetail(playlistId) {
    ...
    try {
        // Mostrar indicador de carga de manera segura
        safeElementOperation('playlistInfo', element => {
            element.innerHTML = `
        ...
        // Actualizar título del modal de manera segura
        safeElementOperation('playlistDetailTitle', element => {
            element.textContent = playlist.title || 'Sin título';
        });
        
        // Actualizar descripción de manera segura
        safeElementOperation('playlistDetailDescription', element => {
            element.textContent = playlist.description || 'Sin descripción';
        });
        
        // Actualizar fecha de creación de manera segura
        safeElementOperation('playlistDetailDate', element => {
            element.textContent = `Creada: ${formatDate(playlist.creation_date)}`;
        });
        
        // Actualizar fecha de expiración de manera segura
        safeElementOperation('playlistDetailExpirationDate', element => {
            if (playlist.expiration_date) {
                const expired = isExpired(playlist.expiration_date);
                element.className = `badge ${expired ? 'bg-danger' : 'bg-info'}`;
                element.textContent = `${expired ? 'Expiró' : 'Expira'}: ${formatDate(playlist.expiration_date)}`;
            } else {
                element.className = 'badge bg-secondary';
                element.textContent = 'Sin fecha de expiración';
            }
        });
        
        // Actualizar estado de la playlist de manera segura
        safeElementOperation('playlistDetailStatus', element => {
            const isActive = isPlaylistActive(playlist);
            element.className = `badge ${isActive ? 'bg-success' : 'bg-danger'}`;
            element.textContent = isActive ? 'Activa' : 'Inactiva';
        });
        
        // Configurar botón de descarga de manera segura
        safeElementOperation('playlistDownloadBtn', element => {
            element.onclick = () => {
                window.location.href = `${API_URL}/playlists/${playlistId}/download`;
            };
        });
        
        // Configurar botón de editar de manera segura
        safeElementOperation('editPlaylistBtn', element => {
            element.onclick = () => {
                preparePlaylistForEditing(playlist);
            };
        });
        
        // Configurar botón para eliminar playlist de manera segura
        safeElementOperation('deletePlaylistBtn', element => {
            element.onclick = () => {
        ...
        // Mostrar videos en la playlist de manera segura
        safeElementOperation('playlistVideos', element => {
            let videosHTML = '';
            
            if (!playlist.videos || playlist.videos.length === 0) {
                videosHTML = '<tr><td colspan="4" class="text-center">No hay videos en esta lista</td></tr>';
            } else {
                // Generar HTML para cada video
                playlist.videos.forEach(video => {
                    const videoExpired = video.expiration_date && isExpired(video.expiration_date);
                    videosHTML += `
                        <tr class="${videoExpired ? 'table-danger' : ''}">
                            <td>${video.title || ''}</td>
                            <td>${video.description || '<span class="text-muted">Sin descripción</span>'}</td>
                            <td>
                                ${video.expiration_date ? 
            ...
            element.innerHTML = videosHTML;
        });
        
        // Cargar videos disponibles para agregar, dispositivos asignados y disponibles
        try {
            await loadAvailableVideos(playlistId, playlist);
            await loadPlaylistDevices(playlistId);
            await loadAvailableDevices(playlistId);
        } catch (e) {
            console.error("Error al cargar datos relacionados:", e);
        }
        
        // Configurar botón para agregar video de manera segura
        safeElementOperation('addVideoBtn', element => {
            element.onclick = () => {
                const videoSelect = document.getElementById('addVideoSelect');
        ...
        // Configurar botón para agregar dispositivo de manera segura
        safeElementOperation('addDeviceBtn', element => {
            element.onclick = () => {
        ...
        // Mostrar el modal de manera segura
        ...
        if (modal && window.bootstrap) {
            try {
                ...
                console.error("Error al mostrar modal:", e);
                throw new Error("Error al mostrar modal de detalles: " + e.message);
            }
        } else {
            console.error("No se encontró el modal de detalles de playlist o Bootstrap no está disponible");
            showToast("Error: No se pudo mostrar los detalles de la playlist", 'error');
        }
...
async function savePlaylistChanges() {
    try {
        ...
        // Obtener el ID de la playlist de manera segura
        const editIdInput = document.getElementById('editPlaylistId');
        ...
        // Obtener los valores de los campos del formulario de manera segura
        const editTitleInput = document.getElementById('editPlaylistTitle');
        const editDescriptionInput = document.getElementById('editPlaylistDescription');
        const editExpirationInput = document.getElementById('editPlaylistExpiration');
        const editActiveCheckbox = document.getElementById('editPlaylistActive');
        ...
        console.log("Enviando datos:", playlistData, "a URL:", `${API_URL}/playlists/${playlistId}`);
        ...
        // Cerrar el modal de edición de manera segura
        if (window.bootstrap) {
            try {
                const editModal = document.getElementById('editPlaylistModal');
                if (editModal) {
